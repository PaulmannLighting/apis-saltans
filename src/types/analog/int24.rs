...
impl Int24 {
    ...
    ///
    /// Big-endian representation of `0x800000`.
    pub const NON_VALUE: [u8; 3] = [0x80, 0x00, 0x00];

    /// Crate a new `Int24` from an `I24` value.
    #[must_use]
    pub fn new(value: I24) -> Option<Self> {
        if value == I24::from_be_bytes(Self::NON_VALUE) {
            None
        } else {
            Some(Self(value))
        }
    ...
    pub fn non_value() -> Self {
        Self(I24::from_be_bytes(Self::NON_VALUE))
    }
}

impl From<Int24> for Option<I24> {
    fn from(value: Int24) -> Self {
        if value == Int24::non_value() {
            None
        } else {
            Some(value.0)
        }
