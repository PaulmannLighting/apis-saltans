...
impl From<MeasuredValue> for Type {
    fn from(value: MeasuredValue) -> Self {
        value.0.into()
    }
}

impl TryFrom<Uint16> for MeasuredValue {
    type Error = Uint16;
...
impl TryFrom<Type> for MeasuredValue {
    type Error = Type;

    fn try_from(value: Type) -> Result<Self, Self::Error> {
        Uint16::try_from(value).map(Self)
    }
}
