        ...
        T: Iterator<Item = u8>,
    {
        ...
        match tag {
            ...
            tag if tag & MASK == 0x0020 || tag & MASK == 0x0030 => {
                Settings::from_le_stream_tagged(tag, bytes).map(Self::Battery)
            }
            tag if tag & MASK == 0x0040 || tag & MASK == 0x0050 => {
                Settings::from_le_stream_tagged(tag, bytes).map(Self::Battery2)
            }
            tag if tag & MASK == 0x0060 || tag & MASK == 0x0070 => {
                Settings::from_le_stream_tagged(tag, bytes).map(Self::Battery3)
            }
            _ => None,
        }
    }
