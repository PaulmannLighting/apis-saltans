...
generated by declarative macros under `src/macros/`, while hand-written code is kept for behavior that
...
- `src/command.rs` and `src/command/` define command traits:
  ...
  - `Scoped` for command scope; cluster-specific commands receive it through a blanket implementation.
...
- `src/macros.rs` re-exports the crate-local declarative macros from `src/macros/` used by command, command enum,
  attribute value, and attribute table modules.

## Runtime Command Flow

The coordinator receives APSDE `DataIndication<Bytes, T, K>` values. Its mux extracts the
cluster ID from indication metadata and calls `Frame<Cluster>::parse(cluster_id, bytes)`.
The ZCL crate also supports `TryFrom<zb_aps::Data<Bytes>>` for callers working with raw APS
data frames. Both entry points use the same parsing flow:
...
5. The `parse_zcl_frame` method generated by `zcl_command_enum!` selects the command payload
   by command ID and accepted direction, then parses it with `FromLeStream`.

Outgoing commands are serialized from their concrete payload types:

1. A command payload implements `ToLeStream` plus the command metadata traits.
2. `UnsequencedFrame<Bytes>::from_command` serializes the payload and derives its header metadata.
3. The sender assigns a transaction sequence with `into_frame`.
4. `Frame<Bytes>` serializes the header followed by the already encoded payload.

The runtime command enums and `Cluster` are incoming dispatch containers; they do not implement
`ToLeStream`. Callers can extract concrete payloads through the generated `TryFrom<Cluster>`
...
The macros under `src/macros/` are crate-local and re-exported with `pub(crate) use`. They are intended
...
- `zb_core::ClusterSpecific<zb_core::Cluster>` for cluster-specific commands.
...
- `Scoped` with `SCOPE = Scope::Global` for global commands.
...
- A crate-local `parse_zcl_frame` method for dispatch by command ID and direction.
- `From<Payload> for Command` for each payload.
- `From<Command> for crate::Cluster`.
- `zb_core::ClusterSpecific<zb_core::Cluster>` for cluster-specific command enums.
...
- `ClusterSpecific<zb_core::Cluster>` and `Profiled` impls for all generated enums.
...
`src/macros/`, with separate modules for commands, command enums, attribute newtypes, attribute tables,
bitflag formatting, and profile defaults. Attribute macro tests live in `macros/tests.rs`.
