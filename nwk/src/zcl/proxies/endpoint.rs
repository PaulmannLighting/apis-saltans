...
impl<T> Proxy<T> {
    ...
    #[must_use]
    pub const fn new(inner: T, pan_id: u16, endpoint: Endpoint) -> Self {
        Self {
            inner,
            pan_id,
            endpoint,
    ...
    where
        ...
        F: Into<Frame> + Send,
    {
        self.inner.send(self.pan_id, self.endpoint, frame).await
    }
    ...
    {
        self.inner
            .communicate(self.pan_id, self.endpoint, frame)
            .await
    }
}
