...
ZCL returns the deferred `HwResponse` for every internally generated OTA command. The server routes
commands to one long-lived task per destination and tracks only those destination tasks. Each
destination task owns its transmission operations and any paced page-transfer operation. Page
transfers poll their block responses sequentially and stop the remaining page stream if a
transmission fails.

Drivers must honor `zb_hw::Metadata::tx_options()`. Normal OTA replies include
`TxOptions::ACKNOWLEDGED_TRANSMISSION`; block responses generated by Image Page Request use empty
transmission options as required by ZCL.
