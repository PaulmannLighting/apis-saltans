...
fn discovery_deadline_expires_an_unanswered_offer() {
    run_test(async {
        ...
        let completion = schedule_for(&ota_sender, test_address(), test_image(), timeouts).await;
        receive_zcl(&mut zcl_receiver).await;
...
fn handles_query_block_and_upgrade_end_flow() {
    run_test(async {
        ...
        let image_size = u32::try_from(image.len()).expect("test image length fits OTA size field");
...
async fn schedule(sender: &tokio::sync::mpsc::Sender<Message>, image: Image) -> ScheduledUpdate {
...
fn test_metadata(profile: Profile) -> IndicationMetadata<(), ()> {
    ...
    IndicationMetadata::new(
        ReceivedDestination::Network {
            address: NetworkAddress::new(LOCAL_NWK_ADDRESS).expect("coordinator address is valid"),
            endpoint,
        },
        test_source(),
        profile.as_u16(),
...
async fn receive_raw_zcl(receiver: &mut tokio::sync::mpsc::Receiver<zcl::Message>) -> zcl::Message {
